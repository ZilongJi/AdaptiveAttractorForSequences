...
toc
%%
plot(amplitude,'b','linewidth',2),hold on
plot(amplitude,'r.','markersize',20)
xticks(1:3:length(mbar));
xticklabels(round(mbar(1:3:end)/48,3));
xlabel('Adaptation strength m')
ylabel('Theta sweeps amplitude', 'FontName', 'Arial', 'FontSize', 20)
% 设置坐标轴的字体和大小
set(gca, 'FontName', 'Arial', 'FontSize', 20);
set(gca, 'LineWidth', 1.5);
figure
plot(omega,'b','linewidth',2),hold on
plot(omega,'r.','markersize',20)
xticks(1:3:length(mbar));
xticklabels(round(mbar(1:3:end)/48,3));
xlabel('Adaptation strength m')
